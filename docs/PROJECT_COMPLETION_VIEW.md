...
According to the current progress board, the documented roadmap through `U5` is now complete.

That means there are no unfinished sprints left inside the currently documented roadmap itself,
and the next direct follow-up moves into the narrower fallback-validation criteria stage such as `U6 - Same-Node Required vs Preferred Validation`.
...
- the next step is to decide the required-vs-preferred boundary and how controller-owned resolution should be raised from here
...
- the next step is to define whether the required selector should remain or be downgraded to preferred placement
