...
According to the current progress board, the documented roadmap through `U3` is now complete.

That means there are no unfinished sprints left inside the currently documented roadmap itself,
and the next direct follow-up moves into the fallback-semantics stage such as `U5 - Dynamic Fallback Validation Entry`.
...
- the next step is to decide how same-node-preferred / remote-fallback semantics and controller-owned resolution should be raised from here
...
- the next step is to open remote-fallback semantics as the next direct validation question
