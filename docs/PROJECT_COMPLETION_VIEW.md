...
17. fixing that remote-capable resolution is an artifact-aware policy step that reads `consume policy + producerNode + replicaNodes/recorded order + observable failure signal`
...
According to the current progress board, the documented roadmap through `U10` is now complete.

That means there are no unfinished sprints left inside the currently documented roadmap itself,
and the next direct follow-up moves into the controller-resolution stage such as `U11 - Controller-Owned Placement Resolution Entry`.
...
- `U10` now fixes the remote-capable resolution input set
- the next step is to decide whether this judgment should be raised into a controller-owned resolution path
...
- `U10` then fixed that the remote-capable resolution path should read `consume policy + producerNode + replicaNodes/recorded order + observable failure signal`
- the next step is to define where that judgment should live
