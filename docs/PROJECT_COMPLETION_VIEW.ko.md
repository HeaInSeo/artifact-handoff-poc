...
현재 진행판 기준으로 `U8`까지의 문서화된 로드맵은 모두 완료됐다.

즉 현재는 문서화된 현재 로드맵 안의 미완료 스프린트가 없고,
다음 직접 후속은 `U9 - K8s Observable Integration Validation`처럼 observable integration 경계를 더 좁히는 단계다.
...
- 다음 단계는 K8s observable integration과 remote-capable resolution, 그리고 controller-owned resolution을 어떻게 올릴지 정하는 것
...
- 다음 단계는 이 흐름을 current K8s observable path에 어떻게 연결할지 정하는 것
