...
현재 진행판 기준으로 `U9`까지의 문서화된 로드맵은 모두 완료됐다.

즉 현재는 문서화된 현재 로드맵 안의 미완료 스프린트가 없고,
다음 직접 후속은 `U10 - Remote-Capable Resolution Entry`처럼 remote-capable resolution 입력을 더 좁히는 단계다.
...
- 다음 단계는 remote-capable resolution과 controller-owned resolution을 어떻게 올릴지 정하는 것
...
- 다음 단계는 remote-capable resolution을 어떤 policy input으로 열지 정하는 것
